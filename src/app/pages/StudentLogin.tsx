...
import { UserCircle, Lock, User, ArrowRight, Eye, EyeOff } from "lucide-react";
...
export default function StudentLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    const result = validateLogin(email, password, "student");
    setIsLoading(false);
      ...
      <div className="hidden lg:flex lg:w-1/2 relative bg-indigo-600 overflow-hidden items-center justify-center p-12">
        ...
        <div className="relative z-10 text-white max-w-lg">
          ...
          >
            ...
            <h1 className="text-5xl font-bold mb-6 leading-tight">Welcome back to <br/>FearOff Student.</h1>
            <p className="text-xl text-indigo-100 font-medium leading-relaxed">
              Access your learning materials, submit assignments, and track your progress in one beautifully designed portal.
            </p>
          </motion.div>
        </div>
      </div>
        ...
        >
          <Card className="w-full bg-white/70 dark:bg-[#111111]/70 backdrop-blur-2xl border-slate-200/60 dark:border-white/10 shadow-2xl dark:shadow-none rounded-3xl overflow-hidden">
            ...
            <CardHeader className="pt-10 pb-6 px-8 text-center">
              <CardTitle className="text-3xl font-bold text-slate-900 dark:text-white mb-2">Student Login</CardTitle>
              <CardDescription className="text-base text-slate-500 dark:text-slate-400">Sign in to your account to continue</CardDescription>
            </CardHeader>
            <CardContent className="px-8 pb-8">
              <form onSubmit={handleLogin} className="space-y-5">
                ...
                <div className="flex items-center space-x-2 ml-1">
                  <input type="checkbox" id="remember" className="rounded border-slate-300 text-blue-600 focus:ring-blue-500 bg-slate-50 dark:bg-white/5 dark:border-white/10" />
                ...
                <Button
                  type="submit"
                  disabled={isLoading}
                  className="w-full h-14 text-lg font-medium rounded-2xl bg-blue-600 hover:bg-blue-700 dark:bg-blue-600 dark:hover:bg-blue-500 text-white shadow-xl shadow-blue-500/20 transition-all hover:scale-[1.02]"
                >
                  {isLoading ? (
            ...
            <CardFooter className="flex flex-col border-t border-slate-100 dark:border-white/5 py-6 gap-3">
              <p className="text-sm text-slate-500 dark:text-slate-400 text-center">
                Don't have an account? <Link to="/student-register" className="text-blue-600 dark:text-blue-400 font-semibold hover:underline">Create one</Link>
              </p>
